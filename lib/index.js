...
class SecoKeyval {
  ...
  set(key, val) {
    ...
    return _asyncToGenerator(function* () {
      ...
      const data = Buffer.from(JSON.stringify(_this2._data));
  ...
  changePassphraseOnNextWrite(newPassphrase) {
    if (!this.hasOpened) throw new Error('Must open first.');
    this._seco = (0, _secoRw2.default)(this.file, newPassphrase, this.header);
  }
  ...
  changePassphrase(newPassphrase) {
    ...
    return _asyncToGenerator(function* () {
      _this4.changePassphraseOnNextWrite(newPassphrase);
